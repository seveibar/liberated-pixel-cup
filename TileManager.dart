...
class TileManager {
  ...
  void render(html.CanvasRenderingContext2D c,Camera camera){
    ...
    for(int i = -1;i<c1;i++){
      ...
      final num txi_i = txi+i;
      for(int u = -1;u<c2;u++){
        final int index = ((txi_i)%CHUNK_JOIN) + ((tyi+u)%CHUNK_JOIN)*CHUNK_JOIN;
        if (renderChunkCoordinates[index] != null && renderChunkCoordinates[index].at(txi_i,tyi+u) && renderChunks[index] != null){//TODO fill renderChunkCoordinates beforehand so we dont need to check null
          final num dx = tx + i * (CHUNK_SIZE * GRAPHIC_BLOCK_SIZE);
          c.drawImage(renderChunks[index], dx , ty + u * (CHUNK_SIZE * GRAPHIC_BLOCK_SIZE));
        }else{
          renderChunkCoordinates[index] = new Vec2(txi_i,tyi + u);
          renderChunks[index] = generateTileChunk((txi_i)*CHUNK_SIZE,(tyi + u)*CHUNK_SIZE);
        }
      }
    }
  }
